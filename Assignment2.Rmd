...
tridiag_matrix <- function(n) {
  ...
  diag_vals <- sqrt(1:(n-1) / 2)
  matrix <- diag(0, n)
  diag(matrix[-1, -n]) <- diag_vals
  diag(matrix[-n, -1]) <- diag_vals
  matrix
}
...
numbers <- c(0:8, 437681258)
vec_to_binary(numbers)
...
```{r}
# Function to convert integer to any base
int_to_base <- function(n, base = 2) {
  if (n == 0) return(0)
  result <- integer(0)
  while (n > 0) {
    result <- c(n %% base, result)
    n <- n %/% base
  }
  return(result)
}
