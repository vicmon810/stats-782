...
```{r}
compute_limit <- function(max_n = 100,threshold = 1e8) {
  ...
  for (n in 1:max_n) {
    ...
    if (diff_from_e > threshold) { # stop when the diff_from_e is reach the floating point and turn into inf
      break
    }
  }
  # plot the performance 
  plot(results$n, results$value, type = "l", col = "blue", 
...
# TEST comment out because running too long
#set.seed(782)
#count = 0
#repeat{
#theta <- runif(1, 0,100) #set theta with random value 

#sum_pmf <- verify_pmf_sum(theta, count)
#cat("Sum of PMF values:", sum_pmf, "with theta value", theta, "and x vlaue" ,count, "\n") # 
#count <- count +1
#if (count > 1e6) break
#}
