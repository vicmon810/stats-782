...
proportion_females_with_acne <- nrow(xs.nz[(!is.na(xs.nz[,"acne"]) & xs.nz[, "sex"] == "F" & xs.nz[, "acne"] == 1),])/ nrow(xs.nz["acne"])
...
```{r}
sum_series <- function(n) sum(1 / (1:(n-1) * 2:n))
...
```{r}
prod_series <- function(n) prod(sapply(1:n, function(k) 1 + (-1)^(k+1) / k))
